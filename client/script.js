...
function makeLeaderboard(users, gen) {
  let code = '<button onclick="location.reload();" class="back">Back</button><br/><br/><br/><div  class="Leaderboard">';
  for (u of users) {
    let data = gen(u);
...
function levelLeaders(file, level) {
  ...
  makeLeaderboard(users, u => {
    ...
    return `<span style="font-size:${ftsize}px;font-weight:${ftweight};color:${usercolor}">${u.name}:</span> &nbsp; ${data.score}% in ${data.time / 1000} seconds.`;//FIX LATER
    //add colors later
  });
}
...
Leaderboard.onclick = function() {
  ...
  makeLeaderboard(users, u => {
    ...
    return `<span style="font-size:${ftsize}px;font-weight:${ftweight};color:${usercolor};" class="player-name">${u.name}:</span> &nbsp;<span style="color:${avgcolor};">${Math.floor(u.avg)}% average</span>, &nbsp;<span style="color:${gamescolor};">${u.games} games played</span>, &nbsp;<span style="color:${scorecolor};">total score of ${u.score}</span>.`;
  });
}