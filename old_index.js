/*
fs.writeFile("Games.json", Games, function(err) {
  if (err) {
    console.log(err);
  }
});*//*
...
async function updateLeaderboard() {
  ...
  for (uID of keys) {
    ...
    u.totalTime = 0;
    console.log(u.totalTime)
    for (name in Games) {
      game = Games[name];
      for (levelN in game) {
        level = game[levelN];
        console.log(level.worstTime)
        if (!u.completed[name] || !u.completed[name][levelN]) {
          u.totalTime += 100 - level.worstTime;
        }
        else {
          u.totalTime += (u.completed[name][levelN].score - u.completed[name][levelN].time);
        }
      }
    }
    console.log(u.totalTime);
  ...
  Leaderboard.sort((u1, u2) => {
    return u1.totalTime - u2.totalTime;
  });
}
  ...
  for (uID of keys) {
    ...
    for (file in u.completed) {
      for (level in u.completed[file]) {
        ...
        } else if (comp.time > Games[file][level].worstTime) {
          console.log('add', comp.time)
          Games[file][level].worstTime = comp.time;
        }
      }
    }
    db.set(uID, u).then(() => {});
...
io.on('connection', async (socket) => {
  ...
  let user = await db.get(userID).catch(() => {
  ...
  socket.emit('Data', user, unlockedGames, allUsers, Leaderboard);
